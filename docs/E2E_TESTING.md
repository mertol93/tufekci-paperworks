...
5. Non-blank PDF.js canvas pixels and progressive text search across the edited plan.
   The visible streaming text layer publishes its completed text to the retryable
   per-source-page search cache, and the case waits for that render before searching. Pure
   boundary tests separately cover selected-locale casing, compatibility normalisation,
   rendered/extracted whitespace parity, rejected-cache eviction, and content-free errors.
